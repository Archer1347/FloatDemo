...
import com.archer.floatwindow.R
import com.archer.floatwindow.util.BaseLifecycle
import com.archer.floatwindow.util.appName
    ...
    if (!canDrawOverlays(fragmentActivity)) {
        suspendCoroutine<Unit?> {
            showPermissionDeniedDialog(
                ...
                permissionDeniedMessage = String.format(fragmentActivity.getString(R.string.overlays_permission), fragmentActivity.appName),
                onDenied = { message ->
...
private fun showPermissionDeniedDialog(
    ...
    val dialog = AlertDialog.Builder(activity)
        ...
        .setCancelable(false)
        .setNegativeButton(R.string.overlays_permission_no) { dialog, _ ->
            dialog.dismiss()
            onDenied.invoke("取消申请弹窗权限")
        }
        .setPositiveButton(R.string.overlays_permission_yes) { dialog, _ ->
            dialog.dismiss()
            if (gotoSettings()) {
...
private fun whenNextResume(fragmentActivity: FragmentActivity, onResume: () -> Unit) {
    val observer = object : BaseLifecycle {
